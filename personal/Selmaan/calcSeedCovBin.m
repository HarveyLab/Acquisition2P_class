function calcSeedCovBin(obj,radiusPxCov,seedBin,temporalBin,sliceNum,channelNum,writeDir)
...
if isempty(obj.indexedMovie)
    error('No binary movie file associated w/ this acquisition'),
end

binFile = memmapfile(obj.indexedMovie.slice(sliceNum).channel(channelNum).fileName,'Format','uint16');
mov = binFile.Data;
movSize = size(meanRef(obj));
mov = reshape(mov,movSize(1)*movSize(2),[]);
...
% Delete movie variable before getting rid of memory map to prevent loading
% full variable into memory!
clear mov
