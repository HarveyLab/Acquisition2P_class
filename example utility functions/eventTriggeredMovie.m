function [avgMov, dFmov] = eventTriggeredMovie(obj,avgMovFrames,nSlice,nChannel)
...
fileFrames = cumsum(obj.correctedMovies.slice(nSlice).channel(nChannel).size(:,3));
...
for frame = 1:length(allFrames)
    if thisFile ~= fileFrameIDs(frame)
        ...
        t = ScanImageTiffReader(obj.correctedMovies.slice(nSlice).channel(nChannel).fileName{thisFile});
